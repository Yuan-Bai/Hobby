...
public class Slimer extends Monster{
    int aggressivity = 1;//怪物攻击力
    int hp0 = 15;
    int hp = 15;//怪物血量
    int speed = 2;//怪物速度
    String name = "史莱姆";//怪物名字
    int cardqtity = 15;//怪物卡牌初始携带量
    int attackcard = 10;
    int curecard = 0;
    int armorcard = 4;
    int armor = 7;
    ...
    public void action (Character c){
        if(hp <= 0)return;
        if (mc[0] == null){
            System.out.println("史莱姆企图逃跑了");
            return;
        }
        if (mc[0] instanceof ArmorCard){
            ArmorCard a = (ArmorCard)mc[0];
            this.armor += a.armorvalue;
            System.out.println("史莱姆使用了护甲牌,增加了" + a.armorvalue + "点护甲值");
            ...
            if (c.getArmor() >= 0){
                ...
                if (d > c.getHp0()){
                    //y.armor = d - per.bai.cardgame.YuanBai.hp0;
            ...
            System.out.println("史莱姆发动了攻击,并造成" + (attackCard.aggressivity + aggressivity) + "点伤害");
        }else if (mc[0] instanceof CureCard){
            int oldhp = this.hp;
            CureCard cureCard = (CureCard)mc[0];
            this.hp += cureCard.curevalue;
            System.out.println("史莱姆治疗了自己，并增加了" + (this.hp - oldhp) + "点血量");
        }
        remove(0);
        //cardqtity--;
    }
