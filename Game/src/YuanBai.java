...
public class YuanBai implements Character{
    ...
    //人物动作。
    /*public void Action(Card a,Slimer s) {
        if (a instanceof AttackCard){
            AttackCard attackCard = (AttackCard)a;
            if (s.armor >= 0){
                int sum = s.hp + s.armor;
                int d = sum - attackCard.aggressivity - aggressivity;
                if (d > Slimer.hp0){
                    s.armor = d - Slimer.hp0;
                }else {
                    s.hp  = d;
                    s.armor = 0;
                }
        ...
        }else if (a instanceof ArmorCard){
            ArmorCard armorCard = (ArmorCard)a;
            this.armor += armorCard.armorvalue;
            System.out.println("你使用了护甲牌");
        }else if (a instanceof CureCard){
            CureCard cureCard = (CureCard)a;
